...
func (as *AuthServer) CreateToken(ar *authRequest, ares []authzResult) (string, error) {
	...
	fmt.Print("Expiration :", tc.Expiration)
	fmt.Print("Access :", []*token.ResourceActions{})
	fmt.Print("JWT ID :", fmt.Sprintf("%d", rand.Int63()))
	...
	for _, a := range ares {
		fmt.Println("Actions :",a.autorizedActions)
	...
	fmt.Println("Token before signing :", payload)
	...
	fmt.Println("Token after signing :", payload)
